    ...
    if adc_read.count(i) == 1:
        ...
        while(range_max == -1):
            if i_max > max(adc_read):
                ...
                i_max = i_max + 1
        print('  i_min: ' + str(i_min) + ' range_min: ' + str(range_min) +
              ' i_max: ' + str(i_max) + ' range_max: ' + str(range_max))
        # Interpolation
        adc_V_lookup.append(
            DAC_QUANTUM * (range_min + ((range_max - range_min)/(i_max-i_min))*(i-i_min)))
    else:
        ...
        # Collapsing
        adc_V_lookup.append(
            DAC_QUANTUM * ((range_max - range_min)/2 + range_min))
